...
for i in $(seq 1 $new_index); do
    ...
    command4="if ! grep -q 'UNSATISFIABLE' '$file'; then ./3-cube-merge-solve-iterative-cc.sh -c $n $child_instance '$d/$v-$i' $a $a; elif grep -q 'VERIFIED' '$vfile'; then find $cube_file$i.* | grep -v '$cube_file$i.adj.simp.verify\|$cube_file$i.adj.simp.log\|$cube_file$i.adj.simplog' | xargs rm; fi"
