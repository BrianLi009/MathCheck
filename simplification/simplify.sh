...
./cadical-ks/build/cadical-ks "$f" "$f.drat" --order $o --unembeddable-check 17 -o simp/"$f".simp1 -e simp/"$f".ext1 -n -t $m | tee log/"$f".simp1
...
do
	time_left=$(echo $m-$time_used | bc)
	time_left=$(printf "%.0f\n" "$time_left")
	./cadical-ks/build/cadical-ks simp/"$f".simp"$i" simp/"$f".simp"$i".drat --order $o --unembeddable-check 17 -o simp/"$f".simp$((i+1)) -e simp/"$f".ext$((i+1)) -n -t $time_left | tee log/"$f".simp$((i+1))
