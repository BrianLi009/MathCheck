...
i=1
./cadical-ks/build/cadical-ks "$f_dir" "$f_dir.drat" --order $o --unembeddable-check 17 -o "$f_dir".simp1 -e "$f_dir".ext1 -n -c $m | tee log/"$f_dir".simp1
...
while (( conf_used < m && conf_left != 0 )); do
  ./gen_cubes/concat-edge.sh $o "$f_dir".simp"$i" "$f_dir".ext"$i" | ./cadical-ks/build/cadical-ks /dev/stdin "$f_dir".simp"$i".drat --order $o --unembeddable-check 17 -o "$f_dir".simp$((i+1)) -e "$f_dir".ext$((i+1)) -n -c $conf_left | tee log/"$f_dir".simp$((i+1))
  if [ "$s" != "true" ]; then
    ./gen_cubes/concat-edge.sh $o "$f_dir".simp"$i" "$f_dir".ext"$i" | ./drat-trim/drat-trim /dev/stdin "$f_dir".simp"$i".drat -f | tee log/"$f_dir".simp$((i+1)).verify
